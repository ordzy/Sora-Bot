...
async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (!userRoleIds.includes(roleId)) {
    return interaction.reply({ content: 'Only members with the ordzy role can use this command.', ephemeral: true });
  }
  ...
  const roleEditData: any = {
    reason: `ordzy role updated by ${member.user.tag}`,
  };
  ...
  if (imageAttachment) {
    if (imageAttachment.size > MAX_ICON_SIZE) {
      return interaction.reply({
        content: 'The image is too large big bro, upload an image under 2MB. Supported formats: PNG, JPG.',
        ephemeral: true,
      });
    }
