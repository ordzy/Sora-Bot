...
const data = new SlashCommandBuilder()
  .setName('stormr')
  .setDescription('Modifies the storm role’s color, icon, and optionally name.')
  .addStringOption(option =>
    option.setName('hex')
      .setDescription('Hex color (e.g. #E9B4E4)')
      .setRequired(false)
  )
...
async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (!userRoleIds.includes(roleId)) {
    return interaction.reply({ content: 'Only members with the storm role can use this command.', ephemeral: true });
  }
  ...
  const roleEditData: any = {
    reason: `storm role updated by ${member.user.tag}`,
  };
  ...
  try {
    ...
    console.error('Error updating storm role:', error);
    await interaction.reply({
      content: 'Something went wrong while updating the storm role. Please try again or check the image format/size.',
      ephemeral: true,
    });
  }
}
